...
axes.plot(Rarr, theta(Rarr, 2.0)/np.pi*180, label=r"$\theta_c(R)$, a=2.0")
axes.plot(Rarr, theta(Rarr, 1.9)/np.pi*180, color="navy", label=r"$\theta_c(R)$, a=1.9")
axes.plot(Rarr, theta(Rarr, 1.8)/np.pi*180, label=r"$\theta_c(R)$, a=1.8")
...
axes.set_xlabel(r"Grain radius $R$ in [a.u.]", fontsize=20)
...
axes.plot(df_grain["R"], df_grain["THETA [DEG]"], "o", color="red", label="Simulation: Grain")
axes.plot(df_elastic["R"], df_elastic["THETA [DEG]"], "o", color="green", label="Simulation: Elastic restorion")
